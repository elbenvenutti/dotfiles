...
tmux send-keys -t $WORK_WINDOW.5 C-m cd Space $WORKDIR C-m yarn Space run Space test:e2e:watch C-m

tmux new-window -n "$SRV_NAME"
tmux split-window -t $SRV_WINDOW -v
tmux split-window -t $SRV_WINDOW -v

tmux send-keys -t $SRV_WINDOW.1 C-m cd Space $WORKDIR C-m yarn Space run Space dyson C-m
tmux send-keys -t $SRV_WINDOW.2 C-m cd Space $WORKDIR C-m yarn Space run Space start C-m
tmux send-keys -t $SRV_WINDOW.3 C-m cd Space $WORKDIR C-m NODE_ENV=local Space yarn Space run Space start Space -- Space --port=$SECONDARY_PORT C-m
